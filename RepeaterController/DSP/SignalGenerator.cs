...
namespace RepeaterController.DSP
{
    ...
    public class SignalGenerator : ISampleProvider
    {
        ...
        /// <summary>
        /// Frequency for the Generator. (20.0 - 20000.0 Hz)
        /// This is used for SinFunction mode.
        /// </summary>
        public double FrequencyTwo
        {
            get { return frequencyTwo; }
            set { frequencyTwo = value; }
        }
            ...
            for (int sampleCount = 0; sampleCount < count / waveFormat.Channels; sampleCount++)
            {
                switch (Type)
                {
                    ...
                    case SignalGeneratorType.Sweep:
                        ...
                        break;
                    case SignalGeneratorType.SinFunction:
                        {
                            // Sinus Generator
                            multiple = TwoPi * frequency / waveFormat.SampleRate;
                            multiple2 = TwoPi * frequencyTwo / waveFormat.SampleRate;
                            sampleValue = Gain * (Math.Sin(nSample * multiple) + Math.Sin(nSample * multiple2));
