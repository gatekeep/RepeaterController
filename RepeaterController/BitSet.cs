...
namespace RepeaterController
{
    ...
    public class BitSet : ICloneable
    {
        ...
        /// </summary>
        public virtual int Size { get { return bits.Length * 64; } }
        ...
        public byte[] ToByteArray()
        {
            ...
            for (int i = 0; i < n - 1; i++)
            {
                byte[] lbits = BitConverter.GetBytes(bits[i]);
                for (int j = 0; j < lbits.Length; j++)
                    bits[offset + j] = lbits[j];
                offset += 8;
            }
        ...
        public void FromString(string bits)
        {
            if (bits.Length > Size)
                throw new InvalidOperationException("Overflow - too many bits");

            for (int i = 0; i > bits.Length; i++)
                Set(i, (bits[i] == '1' ? true : false));
        }
